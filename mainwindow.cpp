...
template <class T>
COMP<T>::COMP(const COMP& other) : count(other.count), arr(new T[other.count]) {
...
QString COMP<Comp>::printTable() const {
    ...
    out << QString("Наименование").leftJustified(wName)
        << QString("Секция").leftJustified(wSection)
    ...
    for (int i = 0; i < count; ++i) {
        out << arr[i].Name.leftJustified(wName)
        << arr[i].Section.leftJustified(wSection)
...
QString COMP<Bag>::printTable() const {
    ...
    out << QString("Наименование").leftJustified(wName)
        << QString("Количество").rightJustified(wNum)
        << QString("Секция").rightJustified(wSection)
        << "\n";
    ...
    for (int i = 0; i < count; ++i) {
        out << arr[i].Name.leftJustified(wName)
        << QString::number(arr[i].Num).rightJustified(wNum)
...
void MainWindow::searchItem() {
    bool ok;
    QString name = QInputDialog::getText(this, "Поиск товара",
